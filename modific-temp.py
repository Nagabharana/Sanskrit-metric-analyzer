...
class Module1:
	def line_partition(self,f):
		...
		#letter1=['्']
		#letter2=['ा','ि','ी','ु','ू','ृ','ॄ','े','ै','ो','ौ','ं','ः']
		letter1=['्']
		letter2=['ा','ि','ी','ु','ू','े','ै','ो','ं',':','ॄ','ौ','ृ','ः']
		#uniLRange=3202
		#uniHRange=3299
		uniLRange=2309 #sanskrit
		uniHRange=2416
...
class Module2:
	def laghu_guru(self,akshara_list):
		...
		g_letter=['आ','इ', 'ऊ', 'ॠ', 'ऌ','ऐ','ओ','औ','ऑ','ा','ी','ू','ो','ॄ','ै','े','ं',':','ौ','ृ','ः','़ु'
		halant_letter=['्']
		len1=len(akshara_list)
		i=0
		while i<len1 :
...
class Module3:
	...
	def vrutta(self,akshara_list,lg_list,lg_dup) :
		...
		shaardulavikriditha="2221111211122212212"
		...
		if tf == True :
			self.Print_Master(akshara_list,"मगणद्वयम् गुरुद्वयं चेत् विद्युन्माला नाम स्यात् |ವಿದ್ಯುನ್ಮಾಲಾ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),maanavakam)
		if tf == True :
			self.Print_Master(akshara_list,"भगणात् परतः तगणलघुगुरवो यदि स्युः तदा माणवकम् नाम स्यात् | ಮಾನವಕಂ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),halamukhi)
		if tf == True :
			self.Print_Master(akshara_list,"रगणात्परतः नगणसगणो चेत् हनमुखी नाम स्यात् | ಹಲಮುಖಿ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),bhujagashashibrutha)
		if tf == True :
			self.Print_Master(akshara_list,"नगणद्वयम् मगणश्चेत् तदा भुजगशषिभृता नाम स्यात् | ಭುಜಂಗಶಶಿಬೃತ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),shuddavirat)
		if tf == True :
			self.Print_Master(akshara_list,"मगन सगणो, जगणः, गुरुः चेत् शिध्दविराट् नामस्यात् | ಶುದ್ಧವಿರಾಟ್ ವೃತ್ತ ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),champakamala)
		if tf == True :
			self.Print_Master(akshara_list,"भगण, मगण, सगणगुरवश्च यदि भवेयुः चम्पकमाला  स्यात् | ಚಂಪಕಮಾಲಾ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),indravajra)
		if tf == True :
			self.Print_Master(akshara_list,"तगणद्वयम् जगणो गुरुद्वय च यदि भवेयुः तदा इन्द्रवज्रा नाम स्यात् | ಇಂದ್ರವಜ್ರ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),upendravajra)
		if tf == True :
			self.Print_Master(akshara_list,"जगनतगनजगणाः गुरुद्वयं च चेत् उपेन्द्रवज्रा नाम स्यात् | ಉಪೇಂದ್ರವಜ್ರ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),dodhaka)
		if tf == True :
			self.Print_Master(akshara_list,"भगणत्रयात् गुरुद्वयं च चेत् दोधक नाम स्यात् | ಡೋಧಕ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),rathoddhata)
		if tf == True :
			self.Print_Master(akshara_list,"रगण, नगण, रगणात् लघुगुरु चेत् रथोध्दता नाम स्यात् | ರಥೋದ್ಧತಾ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),swaagatha)
		if tf == True :
			self.Print_Master(akshara_list,"रगणनगणभगणा: गुरुद्वयं च चेत् स्वागता  नाम स्यात् | ಸ್ವಾಗತ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),vamshasta)
		if tf == True :
			self.Print_Master(akshara_list,"जगणतगणो जगणरगणो यदि तदा वंशस्तमिति  नाम स्यात् | ವಂಶಸ್ಥ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),indravamsha)
		if tf == True :
			self.Print_Master(akshara_list,"तगण, तगण, जगण, रगणाः स्युर्यदि इन्द्रवज्रा नाम स्यात् | ಇಂದ್ರವಂಶ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),druthavalambitha)
		if tf == True :
			self.Print_Master(akshara_list,"नगणभगणो भगणरगणो च यदि स्युः तदा द्रुतविळम्बितम् नाम स्यात् |ದ್ರುತವಲಂಬಿತ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),malathi)
		if tf == True :
			self.Print_Master(akshara_list,"नगणजगनो जगणरगणो चेत् तदा मालती | ಮಾನವಕಂ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),bhujangaprayatham)
		if tf == True :
			self.Print_Master(akshara_list,"चतुर्भिः यगणो भुजन्गप्रयातो नाम भवेत् |ಭುಜಂಗಪ್ರಯಾತಂ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),priyamvada)
		if tf == True :
			self.Print_Master(akshara_list,"नगण, भगण, जगण, रागणैः प्रियंवदा नाम भवेत् |ಪ್ರಿಯಂವದಾ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),praharshini)
		if tf == True :
			self.Print_Master(akshara_list,"मगणनगणो जगणरगणो गुरुरेकश्च चेत् प्रहर्शिणी नाम भवति | ಪ್ರಹರ್ಷಿಣಿ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),ruchira)
		if tf == True :
			self.Print_Master(akshara_list,"जगणभगणो, सगणजगणगुरवश्च यदा स्युः रुचिरा नाम स्यात् |ರುಚಿರ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),manjubhashini)
		if tf == True :
			self.Print_Master(akshara_list,"सगण, जगण, सगण जगणगुरवश्चेत् मञ्जुभाशिनी नाम स्यात् |ಮಂಜುಭಾಷಿಣಿ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),vasanthatilaka)
		if tf == True :
			self.Print_Master(akshara_list,"तगणभगणो जगणो गुरुद्वयं च चेत् वसन्ततिलकं नाम स्यात् |ವಸಂತತಿಲಕ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),maalini)
		if tf == True :
			self.Print_Master(akshara_list,"नगणद्वय, मगणयगणद्वयैर्युता मालिनी स्यात् |ಮಾಲಿನಿ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),vaani)
		if tf == True :
			self.Print_Master(akshara_list,"नगण, जगण, भगण, जगण, तगणैःगुरुस्म्हितै: चेत् वाणिनो नाम स्यात् | ವಾಣಿ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),shikarini)
		if tf == True :
			self.Print_Master(akshara_list,"यगण, मगण, नगण, सगण, भगणाः लघुगुरु च चेत् शिखरिणी स्यात् | ಶಿಕರಿಣಿ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),harini)
		if tf == True :
			self.Print_Master(akshara_list,"जगणसगणो, जगनसगनभगणाः, लघुगुरू च चेत् प्रुथ्वी नाम स्यात् |ಹರಿಣಿ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),prithvi)
		if tf == True :
			self.Print_Master(akshara_list,"नगण सगण, मगण,रगण, सगणाः , लघुगुरु च चेत् हरिणी नाम स्यात् |ಪೃಥ್ವಿ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),mandakrantha)
		if tf == True :
			self.Print_Master(akshara_list,"मगणभगणो, नगणतगणो, तगणात् परतः गुरु चेत् मन्दाक्राता नाम स्यात् |ಮಂದಾಕ್ರಾಂತಾ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),shaardulavikriditha)
		if tf == True :
			self.Print_Master(akshara_list,"मगण, सगण, जगण, सगण, तगण, तगण, गुरवश्चेत् शार्दूलविक्रीदितम्  नाम स्यात् |ಶಾರ್ದೂಲವಿಕ್ರೀಡಿತ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),maththeebhaavikriditha)
		if tf == True :
			self.Print_Master(akshara_list,"सगण, भगण, रगण, नगण, मगण, यगणाःलघुगुरु च  चेत् तदा मत्तॆभविक्रीदितम् नाम स्यात् |ಮತ್ತೆಭಾವೀಕ್ರಿಡಿತ ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		tf=self.vruttaMaster(copy.deepcopy(lg_list),sragdharaa)
		if tf == True :
			self.Print_Master(akshara_list,"मगण, रागान, भगण, नगणैः, यगणत्रयेण च स्रग्धरा नाम स्यात् |ಸ್ರಗ್ಧರಾ  ವೃತ್ತ",0,lg_list,lg_dup)
			return True
		return False
...
if __name__ == '__main__':	
	...
	f2.close()
	Module1_instance=Module1() #split akshara and normalized output
	akshara_list=Module1_instance.line_partition(f)
	Module2_instance=Module2() #places laghu and guru
